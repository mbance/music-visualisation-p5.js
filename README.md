...
- On line 49 call the `this.addTicks(centreX, bottomY, freqLabel)`
  function correctly specifying the arguments. This will add the ticks
  to the graph. The comments above the `addTicks` function should help
  you work out what each parameter does.
- On line 54 call the `this.addNeedle(energy, centreX, bottomY)`
